    ...
    with open(csvFile, "r") as f:
        ...
        for etudiant in f:
            ...
            for j in range(len(etudiant)):
                if header[j] == "NOM" or header[j] == "PRENOM":
                    s = etudiant[j].replace(" ", "").replace("-", "").lower()
                    plop[-1][header[j]+" ASCII"] = unicodedata.normalize('NFD', s).encode(
                        'ascii', 'ignore').decode('ascii')  # Supprime les accents
                plop[-1][header[j]] = etudiant[j]
    return plop, header
...
def createSetFromName(etudiant, nb_nom, nb_prenom):
    ...
    x = set()
    for i in etudiant['NOM ASCII'][:nb_nom]:
        x.add(i)
    for i in etudiant['PRENOM ASCII'][:nb_prenom]:
        x.add(i)
...
def createCSV(etudiants, nb_nom, nb_prenom, header):
    ...
    with open(csvOut, "w") as f:
        ...
        for etudiant in etudiants:
            ...
            if etudiant["NUMERO"]:
                print(etudiant["NOM"] + " " +
                      etudiant["PRENOM"] + " : " + etudiant["NUMERO"])
            del etudiant["NUMERO"]
            del etudiant["NOM ASCII"]
            del etudiant["PRENOM ASCII"]
